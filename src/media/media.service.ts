    ...
    while (true) {
      const medias = await this.mediaAssetRepository
        ...
        .andWhere(
          `
            (
              (m.refType = :brandType AND NOT EXISTS (
                SELECT 1 FROM brands b WHERE b.id = m.ref_id::uuid AND b.is_active = true
              ))
              OR
              (m.refType = :categoryType AND NOT EXISTS (
                SELECT 1 FROM categories c WHERE c.id = m.ref_id::uuid AND c.is_active = true
              ))
              OR
              (m.refType = :productType AND NOT EXISTS (
                SELECT 1 FROM products p WHERE p.id = m.ref_id::uuid AND p.is_active = true
              ))
            )
          `,
          {
