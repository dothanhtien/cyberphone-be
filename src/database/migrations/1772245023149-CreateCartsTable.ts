...
export class CreateCartsTable1772245023149 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE UNIQUE INDEX "uq_carts_user_id_active" ON "carts" ("user_id") WHERE "user_id" IS NOT NULL AND "status" = 'active'`,
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX "uq_carts_session_id_active" ON "carts" ("session_id") WHERE "status" = 'active'`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`DROP INDEX "public"."uq_carts_session_id_active"`);
